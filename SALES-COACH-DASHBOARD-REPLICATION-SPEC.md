...
- **Score Chart** — horizontal progress rows for the door-pitch dimensions: **Objection**, **Talk / Listen**,
  **Questions**, **Tone**, **Close**.
  *(Corrected 4 September. This used to read "each 0-value until pitches are analyzed", and both implementations
  correctly do the opposite: a dimension that was never scored is ABSENT, not zero. The web filters to dimensions
  with a numeric score — "showing a phantom 0 would be a lie" — and the app does the same and shows an honest
  empty state with a Record action instead. A rep reading "Questions: 0" would conclude they never ask any. The
  spec was the only thing still asking for the phantom zeros.)*
...
- **Brand = amber-gold "Lightbulb" on matte black** (mode-agnostic accent):
  ...
  - **Strictly mono-amber — NO second hue, and NO green anywhere.** Every accent (points, gauge, "on" badges, the
    ...
    *(Corrected 4 September. The root copy of this spec said green was "reserved for door Sales/deals accents",
    which would have introduced a second hue. Checked against the built tokens: `success` is `#FDE047` — pale
    amber — `destructive` is `#CA8A04` — burnt amber — there is no green token at all, and the Sales KPI is
    accented with `border-primary`. The two copies of this file disagreed; this is the true one.)*
    *(4 September, founder decision. The WEB's Scoreboard is the one place that breaks this — its top three use
    `text-amber-500` / `text-slate-400` / `text-orange-600`, a gold-silver-bronze podium. The app does NOT copy
    it: the top three are emphasised by size and a single amber. Asked as an explicit conflict between "app and
    web identical" and "strictly mono-amber"; mono-amber won for the app, and the web keeps its podium. The medal
    metaphor still ships where it reads best — the manager's weekly digest email sends the three medals. If the
    two scoreboards look different to you, that is the decision, not a defect.)*
