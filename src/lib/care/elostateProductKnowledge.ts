...
- KPI Analytics (inside Sales Coach) — a growth-focused analytics dashboard across four layers: sales outcomes (conversion rate, close rate, average deal size, revenue), activity (sessions per day, talk-to-listen), conversation quality (discovery, rapport, objection handling, closing — scored from the call with cited evidence), and — the differentiator — coaching effectiveness, headlined by "Reliance Reduction": needing FEWER live cues over time while performance holds means the coaching is actually working. Every number is measured against the rep's OWN past (not a leaderboard), each metric drills down to the sessions that produced it, and anything without enough data honestly reads "building" — never a guessed figure. Managers can set a monthly deals-won quota per rep, which powers a Quota Attainment metric (deals won this month ÷ target); with no target set it stays "building" — no fabricated goal. Managers get a per-agent team rollup (still growth-framed, not a ranking), including an exception alert that flags a rep whose recent conversion has slipped ≥15% below their own baseline — a "worth a check-in" coaching prompt, never a callout.
