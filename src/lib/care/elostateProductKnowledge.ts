...
- Premium service standard — every C.A.R.E reply (mine and the drafts we hand agents) is written to a genuine relationship-grade standard: it reflects the customer's specific situation so they feel understood, reassures with what was actually checked or done, gives a concrete next step, and adds one proactive thing they didn't ask for but would want. When something has gone wrong on the business's side, we own it plainly and don't offer a grudging half-measure — we either make it right or bring in a teammate who can. The whole aim is that a customer comes away trusting the business more, not less. (This is a behavior baked into how every reply is written; the same standard drives the agent's Co-Pilot and Formulate drafts, so the human agents write to it too.)
...
- Adaptive Knowledge — a company uploads a document (PDF, Word, .txt, .md, .rtf, .odt, .epub, .html) of their own facts (services, hours, pricing, policies, FAQs) and their C.A.R.E AI answers customers from it. It adds facts the AI can rely on; it never changes the AI's honesty rules.
- Customer-assistance guidance — a company can set (or upload a document of) their own rules for HOW the AI should assist their customers — escalation rules, do's and don'ts, brand posture — which shapes how it helps, within its core honesty rules. Distinct from the facts (Adaptive Knowledge) and from what the business represents (product context), each of which also accepts a document upload.
...
- Sales Coach — live sales coaching for reps: a rep runs live in-call coaching OR simply uploads a call recording or phone voice memo (iPhone Voice Memos or Android — the audio is transcribed and the two voices separated automatically), gives an after-pitch review, and tracks skills (tone, pace, talk/listen balance, questions, objection handling, closing) over time against a standard. Reps do a one-time voice enrollment — a quick voice check that stores a pitch reference (a number, never a recording of their voice) so the coach can more reliably tell the rep's side of a conversation from the customer's. Managers make it their OWN coach by writing (or uploading a document — PDF, Word, and plain text/markdown — into) their Coaching Methodology and Product details; what they set there shapes how the coach reasons everywhere it coaches — the live in-call cues, role-play practice, AND the post-call review itself (the after-pitch summary, the skill scoring, and the moment-by-moment breakdown) — and when Product details are set the coach grounds its guidance in the real product and is told to never invent product specifics.
- Meeting Coach — the same live-coaching + recording engine aimed at TEAM MEETINGS (not sales calls): it records the meeting and, afterward, produces a post-meeting Review that measures what the meeting actually PRODUCED — the decisions reached, the action items and WHO owns each (an action with no owner is flagged — the #1 meeting failure), the items left unresolved, and an honest focused-vs-drifted read; if the meeting was prepped with a goal + must-discuss topics it also shows which were covered vs missed. Long meetings are fully supported. The Review can be EXPORTED as a clean, colour-coded PDF (title, summary, clear indicators, each section broken out) to share with the team.
- KPI Analytics (inside Sales Coach) — a growth-focused analytics dashboard across four layers: sales outcomes (conversion rate, close rate, win/loss ratio, average deal size, revenue), activity (sessions per day, talk-to-listen), conversation quality (discovery, rapport, objection handling, closing — scored from the call with cited evidence), and — the differentiator — coaching effectiveness, headlined by "Reliance Reduction": needing FEWER live cues over time while performance holds means the coaching is actually working. Every number is measured against the rep's OWN past (not a leaderboard), each metric drills down to the sessions that produced it, and anything without enough data honestly reads "building" — never a guessed figure. Managers can set a monthly deals-won quota per rep, which powers a Quota Attainment metric (deals won this month ÷ target); with no target set it stays "building" — no fabricated goal. Managers get a per-agent team rollup (still growth-framed, not a ranking), including an exception alert that flags a rep whose recent performance has slipped ≥15% below their own baseline — on either their conversion rate or their pitch quality (a rep can close fine while their craft slips, or the reverse, and a manager coaches both) — a "worth a check-in" coaching prompt, never a callout.
- Sales Arena & Scoreboard (Sales Coach gamification) — the one openly COMPETITIVE layer in the product, and a deliberate exception to the "coaching, never a leaderboard" rule that governs the coaching and KPI surfaces. It turns each scored pitch into points (0–100, from the after-pitch scores) and a band (Elite / Strong / Solid / Developing / Needs coaching), and ranks the team on a live Scoreboard by total points and deals closed. Every rep also gets their own Arena — a personal progress dashboard (a points gauge + band, total points earned, strong-session and deal counts, best pitches, milestone badges, and a per-session trend); on the Macro (door-to-door) mobile app it shares the "Today's Metrics" tab with the field metrics as a two-page swipe. Managers are notified when a rep runs a strong session or closes a deal, and get a calibration tool that hand-scores pitches blind to confirm the AI's scoring is trustworthy before it drives ranks. So yes — unlike the growth-framed coaching views, the Arena IS a leaderboard, by design: competition as motivation. It stays privacy-bounded in one respect — rank and totals are visible to the team, but each rep's per-session score breakdown stays private to them.
- Macro Mode & the door-tracker home (Sales Coach, door-to-door reps) — a rep who works the doors turns on Macro Mode and their mobile home becomes a swipeable two-page screen. Page 0 is the door tracker: three done-of-target dials (doors knocked → presentations → sold) filling toward an AI-calculated daily door target, plus an "Earned today" figure. The target is not typed in — it is worked backward from a manager-set daily SALES goal through the rep's own 30-day close and contact ratios (a stronger closer gets a smaller number), turning an open-ended "just keep knocking" day into a finite, closeable list. Tapping a dial opens the quick-log so each knock is recorded with its real outcome (a presentation is a door where the rep actually spoke to somebody), and the dials always show real logged counts. Swipe left for the rep's normal home. Managers set each rep's daily goal (and, optionally, a dollar value per sale that powers the "Earned today" box) — until a goal is set the screen says so honestly rather than showing a fabricated number.
