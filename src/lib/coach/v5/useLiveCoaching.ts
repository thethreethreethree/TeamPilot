...
export function useLiveCoaching(sessionId: string, context?: SalesContext) {
  ...
  const start = useCallback(async () => {
    ...
    // Voice enrollment (9/2 meeting): the enrolled F0 is NOT seeded into the live pitch cluster yet — the
    // adversarial review found that a hard-anchored seed which differs from the rep's live pitch (calm
    // enrollment vs animated delivery, ≥25 Hz apart) INVERTS attribution, worse than the bootstrap. Founder
    // (2026-09-09) chose to un-wire it until a self-correcting seed is validated. PitchSeparator.seedAgentCentroid
    // remains as the tested primitive for that follow-up. The bootstrap runs unchanged in the meantime.
