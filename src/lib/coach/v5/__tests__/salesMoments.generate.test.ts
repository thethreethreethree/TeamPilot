...
/**
 * generateSalesMoments — the floor changed by the no-minimum-length build (MIN_SEGMENTS 4 → 1).
 *
 * NOTE on why this is needed: afterPitch.generate.test.ts MOCKS generateSalesMoments to test the
 * orchestrator's composition — so it does NOT exercise this floor. The moments engine's own test covers the
 * prompt/parse. So the floor I lowered had no direct guard. This locks it: a short call (below the old floor
 * of 4) reaches the LLM; only a 0-segment transcript short-circuits before it; suppressed / error → EMPTY.
 */
vi.mock("@/lib/claude", () => ({ dissectCoachV5: vi.fn() }));
vi.mock("@/lib/data/salesCoach", () => ({ getCurrentSalesCorpus: vi.fn(async () => null) }));
vi.mock("@/lib/coach/v5/salesMomentsPrompt", () => ({
  buildSalesMomentsSystemPrompt: () => "SYS",
  buildSalesMomentsUserMessage: () => "USER",
}));
vi.mock("@/lib/care/toolPrompts", () => ({ CONVERSATION_IS_DATA: "" }));

import { dissectCoachV5 } from "@/lib/claude";
import { generateSalesMoments } from "../salesMoments";
...
const gen = (segments: ReturnType<typeof seg>[]) =>
  generateSalesMoments({ companyId: "co1", segments } as Parameters<typeof generateSalesMoments>[0]);

beforeEach(() => vi.clearAllMocks());

describe("generateSalesMoments — no minimum length (floor = genuine-empty only)", () => {
