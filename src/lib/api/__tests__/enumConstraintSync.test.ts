...
const PAIRS = [
  ...
  },
  {
    ...
    // DB CHECK on insert (privilege-relevant path). Pinned to 0239 — the migration that now defines
    // the authoritative CHECK (widened from 0008 to the tier roles). REPIN this prefix if a later
    // migration re-alters team_invitations.role. 0239 deliberately puts the CHECK's `role in (...)`
    // FIRST so dbCheck's regex doesn't match the policy's `p.role in (...)` further down.
    name: "team_invitations.role (INVITABLE_ROLES <-> 0239)",
    zod: [...INVITABLE_ROLES],
    db: dbCheck("0239", "role"),
  },
];
