...
export default function Error({
  ...
  return (
    <div className="min-h-screen bg-[#0c0d16] text-[#e8eaf6] flex items-center justify-center px-6">
      <div className="max-w-lg w-full text-center">
        ...
        <p className="text-sm text-[#8895c4] leading-relaxed mb-3">
          The error is shown below. You can try the same step again, or go
          back to the dashboard.
        </p>

        <div className="bg-[#12141f] border border-red-500/30 rounded-xl p-4 mb-6 text-left">
          <p className="text-[10px] uppercase tracking-widest text-red-300 mb-2">
            error
          </p>
          <p className="text-sm text-[#e8eaf6] break-words font-mono">
            {error.message || "Unknown error"}
          </p>
          {error.digest && (
            <p className="text-[10px] text-[#5a6399] mt-3 font-mono">
              digest: {error.digest}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center justify-center gap-3">
          <button
            onClick={reset}
            className="flex items-center gap-2 bg-[#5470ff] hover:bg-[#3a4ff7] text-white font-semibold px-5 py-2.5 rounded-lg transition-colors text-xs"
          >
          ...
          <Link
            href="/dashboard"
            className="flex items-center gap-2 border border-[#252840] hover:border-[#3a3f5c] text-[#8895c4] hover:text-white font-medium px-5 py-2.5 rounded-lg transition-colors text-xs"
          >
