...
import { runWeeklyManagerDigest } from "@/lib/coach/gamification/weeklyDigest";
import { constantTimeEqual } from "@/lib/api/constantTime";

/**
 * GET /api/coach/gamification/weekly-digest-cron — the weekly manager digest (founder 2026-09-04). For each company
 * with points activity in the last 7 days, emails every manager a team rollup (points / strong sessions / deals +
 * top performers). Registered in vercel.json (Mon morning). The companion to the live NotificationBell alerts.
 *
...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await runWeeklyManagerDigest();
