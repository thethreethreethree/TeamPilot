...
/**
 * GET /api/care/durability-sweep-cron — a public Vercel Cron endpoint. Previously untested.
 * Pins the CRON_SECRET auth gate: 503 when the secret is unset, and 401 on a wrong or absent Bearer — a
 * public URL that must NOT be triggerable by anyone. A regression dropping the gate would expose the sweep.
...
describe("GET /api/care/durability-sweep-cron — CRON_SECRET gate", () => {
