...
/**
 * The refresh route proxies Supabase's refresh-token grant server-side (so the extension never needs the
 * Supabase host or the anon key). It has no auth gate — the refresh token IS the credential — so its branches
 * are: rate limited → 429; auth env not configured → 500; upstream renews → 200 with the new tokens; upstream
 * rejects → 401; upstream unreachable → 502. Untested until now.
 */

vi.mock("@/lib/api/rateLimit", () => ({ rateLimit: vi.fn(() => null) }));
vi.mock("@/lib/api/validate", () => ({ readBody: vi.fn(async () => ({ refresh_token: "old-refresh" })) }));

import { POST } from "@/app/api/care/extension/refresh/route";
...
describe("POST /api/care/extension/refresh", () => {
  ...
  it("falls back to the submitted refresh token when upstream omits a new one", async () => {
    vi.stubGlobal("fetch", okFetch({ access_token: "new-access" })); // no refresh_token in response
    const body = await (await POST(req)).json();
    expect(body.refresh_token).toBe("old-refresh"); // reuses the one we sent
  });
