/**
 * Generate a Word document explaining ExecOS for a non-technical business partner.
 * Run:  node scripts/generate-business-overview.js
 * Outputs to docs/ExecOS-Business-Overview.docx
 */
...
children.push(
  h1("1. What it is"),
  p(
    "ExecOS is software that runs alongside the leadership of a company and helps them make better decisions. We describe it as an \"executive operating system\" — the same way Windows or macOS is an operating system for your computer, ExecOS is an operating system for how an executive runs the business."
  ),
  p(
  ...
  p(
    "If you had to explain ExecOS in one sentence, it is this: a system that helps executives diagnose problems honestly, decide carefully, and learn measurably from what actually worked."
  ),
  callout(
...
children.push(
  h1("2. What it solves"),
  p(
    "To understand what ExecOS solves, you have to understand a specific problem that almost every growing company suffers from, often without recognizing it."
  ),
...
children.push(
  ...
  p(
    "ExecOS operates through a specific loop, repeated continuously, that mirrors how a good diagnostic doctor or senior detective actually works. We call it the \"living diagnosis\" loop because the system is constantly cycling through it as new information arrives."
  ),
  ...
  p(
    "Critically: in the first 30 days of any new company's use of ExecOS, the AI is silent. It does not offer suggestions or judgments. It only records and learns. This is intentional. It captures a clean baseline of how the team operates without AI guidance. Then, starting in Month 2, the AI begins offering refined input."
  ),
  p(
...
children.push(
  h1("4. Why it works"),
  p(
    "There are four reasons ExecOS works — meaning, four reasons it produces better outcomes for customers than the alternatives, and four reasons it is defensible as a business."
  ),
  h2("Reason 1 — Honesty becomes the moat"),
  p(
    "Every AI product on the market today is racing to look smarter, more confident, more capable. ExecOS is racing in the opposite direction: to be more honest about what it doesn't know."
  ),
  p(
  ...
  p(
    "ExecOS is the product the market doesn't realize it's about to demand — and once it does, competitors built around \"more confident AI\" will be structurally unable to pivot. Their entire training, prompting, and value proposition is built on the wrong axis."
  ),
  h2("Reason 2 — The data flywheel"),
  p(
    "Every decision made through ExecOS is recorded. Every outcome reviewed. Every method validated or rejected. Over months and years, the per-company memory becomes irreplaceable: it is a structured history of how this specific team actually makes decisions and which ones actually worked."
  ),
  p(
    "This is true switching cost. A customer cannot easily move to a competitor and rebuild that memory — they would lose years of accumulated learning. And the more decisions they run through ExecOS, the better the system gets at advising them specifically. The product gets more valuable to the customer over time. They get more locked in. We get more predictable revenue."
  ),
  ...
  p(
    "ExecOS measures success by whether resolutions held — meaning, did the decision actually produce the predicted outcome, or did the problem reopen? This is what every customer's CFO actually wants to know. It is also what makes the system improve over time: only validated outcomes feed the AI's learning. Garbage acceptance does not become garbage suggestions next time."
  ),
  callout(
    "Every other AI grades its own homework. ExecOS grades by consequence."
  ),
  h2("Reason 4 — Built on a discipline that is hard to copy"),
  p(
    "Anyone can build an AI dashboard. Almost no one can build the diagnostic discipline that ExecOS encodes — because the discipline is not a feature, it is a constitution. The entire system is built so that the rules are enforced structurally (in the database, in the API, in the UI flows) rather than as a checkbox somewhere."
  ),
  p(
  ...
  p(
    "If we are right that the next phase of AI demand is for restraint and honesty (not capability and confidence), ExecOS is positioned to be the category-defining product in executive decision-making. Customers will pay because the cost of wrong decisions is enormous and the existing tools are making it worse, not better. Switching cost will compound. Word of mouth among executives is strong because the product produces a noticeably different outcome — clearer thinking, defensible decisions, durable improvements."
  ),
  p(
    "And, critically, the product gets stronger as the AI ecosystem gets more capable. As the underlying language models (DeepSeek, GPT, Claude, etc.) improve, ExecOS gets even sharper at running its discipline on top of them. We are not in a race against the AI labs — we are running on top of them, channeling their capability through a structure that makes it useful."
  ),
  pageBreak()
);
...
children.push(
  h1("5. How to talk about ExecOS"),
  p(
    "When explaining ExecOS to someone for the first time, here are some sentences that have worked well:"
  ),
  callout(
  ...
  callout(
    "\"Most AI tools today produce confident-sounding output regardless of whether they actually understand the situation. ExecOS is built on the opposite principle — it stays silent until it has real evidence, and even then, the human is always in charge of the decision.\""
  ),
  callout(
  ...
  p(
    "A. ChatGPT will give you a confident answer to any question, regardless of whether it has the information to answer it. ExecOS will not. ExecOS demands evidence before surfacing problems, asks the user for their read before offering its own, and records every learning with full audit trail. Two completely different design philosophies."
  ),
  pmix(
    { text: "Q. ", bold: true },
...
const doc = new Document({
  creator: "ExecOS",
  title: "ExecOS — Business Overview",
  description: "A non-technical overview of the ExecOS product for partners and stakeholders.",
  styles: {
    default: {
      document: {
