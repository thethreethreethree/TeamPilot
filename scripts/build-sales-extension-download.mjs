...
//
// Deterministic: a fixed entry date + sorted entries so a rebuild is byte-identical (no git churn; the
// prebuild hook can regenerate it every deploy without spurious diffs). Uses jszip (already a transitive dep).
