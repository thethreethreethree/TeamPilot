...
const items = [
  ...
  // ─── Coach v4.0 — voice redesign + affirmation surface ───────
  {
    id: "coach-v4.0-warm-chip-header",
    title: "Chip header uses warm first-person voice, not clinical taxonomy",
    instructions: "Type any draft that triggers Coach (e.g. 'this is dumb', 'you guys are making me mad'). Look at the chip's bold header line.",
    expected: "Header reads like a peer noticing something ('I noticed some judgment language', 'I noticed an outside-cause framing', 'I'm picking up some heightened language') NOT a clinical taxonomy label ('Absolute / judgmental phrasing — once in this thread'). The third Coach contract — making the writer feel they've learned — starts at the moment of first contact. A17 lives here.",
    assignee: "partners",
  },
  {
    id: "coach-v4.0-growth-framing-on-recurrence",
    title: "Recurring patterns are framed as practice, not surveillance",
    instructions: "Trigger the same Coach heuristic 2+ times in the same thread (e.g. type 'this is dumb', dismiss, then type 'this is broken'). Look at the tagline below the chip header.",
    expected: "On 2nd occurrence: 'Second time today — you're noticing the shape.' On 3rd+: 'You've noticed this 3 times. That awareness IS the practice.' Reframes the count from surveillance ('3 times in this thread') to growth ('you're building awareness'). Silent on 1st occurrence — no count needed when there's nothing to track yet.",
    assignee: "partners",
  },
  {
    id: "coach-v4.0-demoralizing-badge-removed",
    title: "'Generic template (no LLM read)' badge is GONE",
    instructions: "Force a regex-only Coach fire (type and expand quickly, before LLM 1.2s). Look at the 'Want to try this?' card header.",
    expected: "No badge surfaces when the suggestion is the static template. The text itself is good coaching content; surfacing 'lesser version' framing was demoralizing without serving any user need. When the LLM DID generate a draft-specific revision, a tiny green dot indicator appears next to the header (subtle nod, not a scolding label). User explicitly flagged the v3.12 muted badge as demoralizing.",
    assignee: "partners",
  },
  {
    id: "coach-v4.0-warm-revise-header",
    title: "'How to revise' header is now 'Want to try this?' — invitation, not directive",
    instructions: "Expand any Coach chip. Look at the revision card header.",
    expected: "Header reads 'Want to try this?' — invitation framing. Previously 'How to revise' was instructional, implying the user needed to be told how. Same content; different relationship.",
    assignee: "partners",
  },
  {
    id: "coach-v4.0-autonomy-affirming-dismiss",
    title: "Dismiss button reads 'Send as written' — neutral autonomy, not 'pattern intentional'",
    instructions: "Expand any Coach chip. Look at the dismiss button (secondary, to the right of Refine and revise).",
    expected: "Button reads 'Send as written'. Previously 'Keep as-is — pattern is intentional' implicitly framed sending the original as defiant choice (still keeping the pattern Coach flagged). 'Send as written' is neutral — the writer's choice without grading. A11 mirror frame in tone, not just structure.",
    assignee: "partners",
  },
  {
    id: "coach-v4.0-affirmation-on-sharpen-accept",
    title: "Accepting a Sharpen revision triggers a warm affirmation near the composer",
    instructions: "Type 'could you not act stupid' or similar Coach-triggering draft. Click 'Refine and revise'. In the Sharpen modal, click 'Use the suggestion'.",
    expected: "A small emerald-colored affirmation surfaces above the composer with a sparkle icon — phrasing like 'Nice catch — you saw the pattern and made the move.' or 'That's the practice — noticing, then choosing.' Auto-dismisses after ~4 seconds. The previous toast.info 'Draft updated' is gone — the affirmation IS the feedback for accepting a Sharpen revision. The third Coach contract has a UI surface for the first time. Pre-v4.0 the System tracked accepted suggestions to the §3.1 chain (learning about the writer) but gave the writer nothing back (asymmetric).",
    assignee: "partners",
  },
  {
    id: "coach-v4.0-affirmation-rotates-phrases",
    title: "(JOHN) Affirmation rotates through several phrases, not the same one twice",
    instructions: "(JOHN) Accept Sharpen revisions 3-5 times in quick succession in the same session. Watch the affirmation message each time.",
    expected: "Each acceptance picks a phrase from AFFIRMATION_PHRASES (5 entries currently). The picker uses performance.now() mod len so the user doesn't see the same line twice in a row in normal usage. If user feedback says we still see repeats, expand the phrase list — easier than changing the picker. Per A11, none of the phrases assert the writer was wrong before; they acknowledge the writer's choice without grading.",
    assignee: "john",
  },
  {
    id: "coach-v4.0-warm-verdict-badges",
    title: "Verdict badges read like a peer, not a clinical system",
    instructions: "Expand a Coach chip after the LLM has read it (wait full 1.2s before expanding). Look at the verdict badge next to the source line.",
    expected: "Confirmed: 'I read your full message' (emerald). Uncertain: 'I read it — not 100% sure' (amber). Regex-only: 'Quick pattern catch' (muted). All three are honest; none position the regex-only state as inferior. Previously: 'System read the context · verdict: confirmed' and 'Regex-only · LLM didn't read context' — clinical AND comparative.",
    assignee: "partners",
  },
  {
    id: "coach-v4.0-system-read-card-warmer",
    title: "'System's read on this draft' card now says 'Here's what I'm seeing'",
    instructions: "Type a draft the LLM will confirm with a context_note. Expand the chip.",
    expected: "Card header reads 'Here's what I'm seeing' — first-person, present, peer-voice. Previously 'System's read on this draft' was third-person clinical. Same content; warmer framing.",
    assignee: "partners",
  },
  {
    id: "coach-v4.0-warm-reading-state",
    title: "Reading state says 'Sitting with what you wrote…' not 'Coach reading…'",
    instructions: "Type a draft >12 chars and immediately watch the pre-chip area (before the 1.2s LLM debounce completes). Also expand a chip while LLM is in-flight.",
    expected: "Both the pre-chip pulse and the in-expanded reading state show 'Sitting with what you wrote…' instead of 'Coach reading…' or 'System reading this draft…'. Frames the LLM round-trip as careful attention, not mechanical processing.",
    assignee: "partners",
  },
  {
    id: "coach-v4.0-llm-prompt-warmth-required",
    title: "(JOHN) LLM prompt for revision_suggestion requires warm peer voice",
    instructions: "(JOHN) Read src/lib/claude.ts proposeCoachPatterns prompt section (B). Confirm there is an explicit VOICE block plus updated GOOD examples (warm, peer) and a new BAD example forbidding corrective tone.",
    expected: "Section (B) names the THIRD Coach contract in the prompt itself, instructs warm peer voice, and adds 'You should consider rephrasing this as…' to BAD examples. GOOD examples updated from 'Try naming the specific instance' (instructional) to 'What about: X?' (invitational). The LLM's contribution arrives in the same voice as the rest of v4.0.",
    assignee: "john",
  },
  {
    id: "coach-v4.0-a17-design-against-all-contracts",
    title: "(JOHN) A17 captured to ThinkerThinker — design against ALL contracts simultaneously",
    instructions: "(JOHN) Confirm the v4.0 commit message references A17 and that the ThinkerThinker entry was appended.",
    expected: "A17 in ThinkerThinker names: when a tool serves multiple human contracts, ALL must be design drivers simultaneously, or technical contracts silently optimize against experiential ones. Sibling to A16 (cross-tool composition) but at the within-tool experience altitude. The recurring v3.2 → v3.12 Coach loop was the cost of the experiential contract being absent — that loop closure is the structural evidence A17 was the missing discipline.",
    assignee: "john",
  },
  {
