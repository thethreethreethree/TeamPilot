...
- ✅ **Propose→act loop closed (0820)** — the resolution candidates the plan surfaces are now one-click actionable on all three manager surfaces: Time Off ("cover this gap"), Coverage ("fill this short shift"), Build ("suggest who's free" + conflict warnings). No separate rebuild step.
- ✅ **Phase 6 — manager-entered (0820, founder chose "no login yet")** — printable per-employee **Personal Schedule** (`/dashboard/schedule/personal`, reached from a roster "Schedule" link) + **Availability** enforcement: `AVAILABILITY_SET` was a dead surface (captured, never read); now `isAvailable` is wired into the authority as a block-but-overridable `unavailable` violation and auto-suggest excludes the unavailable. Manager-entered availability editor on the personal page.

## Open founder decisions
Phases 1–5 + 8 are DONE (manager MVP + both import formats + audit + the picker features); Phase 6 is STARTED under the manager-entered model (personal schedule + availability). What awaits you:
...
- **Phase 6** — STARTED as manager-entered (founder decision 2026-08-20: no employee login yet). Done: personal schedule + availability. Remaining under this model: a dedicated **swap** flow (SWAP_APPROVED; the authority path is already built + tested, needs only UI — today a swap is achievable via unassign+assign) and multi-window-per-day availability (v1 = one window/day). Employee **self-service login** remains deferred (chosen against for now). **Phase 7** (make-learning-visible) needs accumulated data.
