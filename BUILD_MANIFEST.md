...
| 5 | Manager/Admin Interface | ✅ **MVP DONE** — ⚠️ import format gap | Roster (API+UI), grid schedule view, file import (CSV: extract→AI-map→preview→atomic commit, +UI), time-off review (evaluate→verdict+candidates+AI proposal→approve/deny, +UI), sub-nav + Sidebar link. All gate-tested backend; UIs browser-verified. **⚠️ LAYER-2 GAP: the import does NOT fit the founder's actual samples** (VA_Weekly_Schedule.docx / VA_Weekly_Color_Grid.pdf = a time-block×staff "On Duty" presence grid, not staff×date shift-codes). Design decision — see `docs/audits/2026-08-19-schedule-import-format-gap.md` (option 1 recommended). OPEN: coverage-requirement editor, VA-shape importer, xlsx/PDF extraction. |
