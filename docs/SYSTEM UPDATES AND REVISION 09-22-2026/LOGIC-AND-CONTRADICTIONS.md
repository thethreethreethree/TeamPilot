...
> **CORRECTED 2026-09-22, while reading this folder in full for the mobile build plan.**
> **B1 does not rest on the image at all.** Its own source table cites `WHATSAPP.txt` — 143 bytes
> of plain text, present in this folder, quoted there verbatim: *"Pages 1-2 will one on the app.
> Page 3 will be on the web version. Page 4 will be the 'score rubric' button that is on page 1
> and 2 (on the app)."* Both clauses of B1 come from that sentence and need nothing else. Only
> **B2** leaned on the image, and the mockups' own sidebars carry it independently.
>
> Both JPEGs in this folder were opened at full resolution on 2026-09-22 and **neither is the
> banner image** described above: `04.50.33` is the four-phone canvas, `05.45.13` is a screenshot
> of the live Training page. So the image this section quarantines is not in the folder and never
> was — which is worth knowing, because a reader looking for it will find two WhatsApp images and
> may reasonably assume one of them is it.
>
> The error in this entry is the one it is itself about: a claim made from context rather than
> from opening the thing. I wrote it, and it sent a settled decision to quarantine for a day.
