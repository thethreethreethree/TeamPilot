...
| **Service-role multi-tenant scoping** | Do service-role routes (RLS bypassed) gate access by company before privileged work? | Coaching `[id]` routes gate via user-client `getSession` (RLS); C.A.R.E via explicit check + user-client fetch (RLS). Both sound. 🟢→✅ **Found + FIXED** (`5e0f935`): 8 C.A.R.E agent routes' explicit company check failed OPEN on a null company (`auth.companyId && …`); now fail-closed. Not a live vuln (RLS covered it) but a defense-in-depth backstop that was off |
