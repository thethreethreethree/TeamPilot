...
class: honesty-thesis silent-truncation on an error edge (§3.4).
severity: low (rare edge, but §3.4 is the product thesis).
...
class: honesty-thesis — a header claim the body denies (§3.4).
severity: medium (founder-visible incident surface).
