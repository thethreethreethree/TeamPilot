...
[
  ...
  { "id": "RQ-F8", "item": "spec 5.6 behavioural RLS test (a peer rep cannot read another rep's pitch/transcript/analysis/summary) still absent — the F1 idempotency test shipped, this one didn't.", "why_skipped": "RLS behaviour needs a live DB / verify:live harness, not a unit mock; the policy text is verified (0215) and rls:audit is green, but there's no behavioural peer-rep test.", "confidence_it_does_not_matter": "low", "opened_at": null, "outcome": null, "trigger": "add to verify:live (SET ROLE peer rep → SELECT returns 0)." },
  { "id": "RQ-F4", "item": "F4 (skip-transcribe-if-exists) is idempotent-by-existence but has no unit gate.", "why_skipped": "The worker's DB-existence check isn't cheaply unit-mockable; behavioural.", "confidence_it_does_not_matter": "medium", "opened_at": null, "outcome": null, "trigger": "a worker integration test with a fake admin client asserting transcribeSpeech is NOT called when a transcript row exists." }
]
```
...
| F3 unvalidated client storagePath → service-role read | MEDIUM | **FIXED** | route rejects a pitch whose `storagePath` isn't prefixed `${companyId}/` (verified buildStoragePath shape, A35) | **GATE**: route.test.ts — "rejects a non-company-prefixed storagePath" |
| F4 retry re-runs paid STT | MEDIUM | **FIXED** | worker gates the transcribe step on transcript-existence, not volatile claim-time status | PROMISE (idempotent-by-existence; behavioural, no unit gate) → residual |
...
**A35 un-named reliance (checked out loud):** (1) F3 relies on `buildStoragePath` → `${companyId}/…` — VERIFIED `assets.ts:194`. (2) F1 relies on `pitches(knock_id)` unique index — VERIFIED `0215:57`. (3) F6/report-card rely on the `pitches`/`rep_pattern_summaries` rep+manager RLS (0215/0218) — verified applied; `rls:audit` 0 tenant-pin risks.
