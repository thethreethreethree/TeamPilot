...
[
  ...
  { "id": "§1.7", "source_file": "CLAUDE.md", "line_range": "230-260", "read_at": "2026-09-03T08:35:00+08:00",
    "why_it_governs": "Ground-up auditing: start at the most foundational layer, because a problem at layer N propagates upward to every layer above it — and an empty flag list at a layer is itself a suspicious finding. Clause 5 is the one that bites: audit findings inform, they do not by themselves halt work.",
    "how_this_build_will_embody_it": "Read because this branch’s commit messages cite it, and it turned out to describe today exactly. The blocked commit was a toolchain-layer fault (CRLF vs a front-matter regex) that I twice mis-diagnosed as a discipline-layer fault (an incomplete ceremony) — the bottom layer propagating upward and being read as the top. Clause 5 also settles the rls:audit failure this commit reports: agent_point_ledger and manager_notifications are flagged, they fail identically on HEAD without this change, and a flag is not a blocker for work it does not touch." },
