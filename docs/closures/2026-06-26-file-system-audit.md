...
| D | UI | Other file actions swallow errors (`openFile`/download silently returns) — same silent-failure class as the delete bug. | MEDIUM | flagged |
| E | UI/Data | Cards show uploader "Unknown" — `cardData` never resolves `uploaderId`→name. (Cap counts by uploader_id and the founder hit 3/3, so attribution records correctly — this is display-only.) | MEDIUM | flagged |
...
1. **E** — resolve uploader name (small, high visibility).
2. **D** — surface errors on download/other actions (same fix as delete).
