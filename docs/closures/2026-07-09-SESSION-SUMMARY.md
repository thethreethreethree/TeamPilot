...
8. Readout/analytics TRUNCATION: layer truncates at PostgREST's 1000-row cap (8+ unbounded queries, care + asset) → wrong metrics for a busy company → bounding pass (per-query limits or DB-side aggregation). [needs row-bound decision — still open]
   **UPDATE 2026-07-09 — precise scope + a reference fix:** `assetReadout.ts` is now **honest-bounded** (b583f5a): its unbounded `files` select (line 82) capped `uploads`/`classifiedUploads`/`casualUploads` + all retrieval metrics silently; added a `bounded` flag + operator warn + a unit test — the number now reads as "capped, undercounts" not silently-wrong (§3.4), WITHOUT pre-empting your fix approach. **Remaining care-side siblings (same class, NOT touched — your decision):** `fetchTeamGrowth` (care.ts:2776) has ~7 unbounded selects feeding aggregation — `support_resolutions`/`support_durability_checks`/`support_ai_co_pilot_edits`/`support_conversations`×2/`support_messages`×2 (`.select("id"/"outcome"/...)` with no limit/count) → the team-growth §3.5 readout undercounts at scale. Apply the same honest-bound flag OR your chosen bounding (per-query limit / DB aggregation). assetReadout is the worked reference.
...
| `0110` | `profiles.experience_mode` column (Standard/Expert dial) | feature foundation — enables the Experience Mode build. Safe anytime; inert until a user picks Standard. Existing users backfilled to Expert (no behavior change) |
| `0111` | guard `companies.ai_guidance_*` (§3.4 control window) against non-admin writes | **§3.4-integrity — a member could `UPDATE companies SET ai_guidance_enabled=true` via direct PostgREST and turn AI guidance ON during the month-1 control window, defeating the honesty baseline. The /api/brain/unlock route is admin-gated but RLS wasn't** |
