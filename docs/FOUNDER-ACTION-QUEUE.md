...
- **Jeff couldn't answer "what is C.A.R.E?" on our own widget — FIXED.** Product knowledge is now authoritative in code (`src/lib/care/elostateProductKnowledge.ts`, checked BEFORE the DB config override so a stale config can't defeat it), comprehensive (all features + channels incl. email + Live Monitor), reaches the LLM un-truncated. **Standing mandate: I update it every feature.** One small your-call flagged: the widget-settings "Product context" field is now display-only for our tenant (labeled as such) — that's intended.
