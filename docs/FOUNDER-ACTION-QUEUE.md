...
6b. **Public widget meters conversations, not messages** (MEDIUM cost / wallet-DoS, found 2026-07-23) — the C.A.R.E embed widget's monthly quota caps CONVERSATION creation, but the operation that costs money is the LLM call PER MESSAGE, and there's no per-tenant monthly message cap (only a per-IP 30/min rate limit, which is per-lambda + IP-spoofable). A holder of a tenant's public embed token can create one in-quota conversation and flood it, running up that tenant's LLM bill largely unbounded. Security model is otherwise sound (origin-validated, session-token-gated, per-call context capped, tenant can pause). **Fix:** add a `countCareMessagesThisMonth` cap in the messages POST — mechanism ready to build; needs the per-plan message-allowance NUMBERS (a pricing decision). Details in `docs/audits/2026-07-23-ground-up-audit-session.md`.
