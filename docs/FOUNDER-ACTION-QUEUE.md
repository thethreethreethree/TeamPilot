...
> - **✅ thesis-trigger guards** — §3.1/§3.2/§3.4 trigger-wiring now live-guarded in verify:live (20 invariants). §3.4 control-window (the honesty moat) built this session (`1c1bc543`); §3.5 durability is the one remaining (deferred deliberately — cron+trigger mix). Revert `1c1bc543` if you'd rather I hadn't; reasoning in its TBC closure.
