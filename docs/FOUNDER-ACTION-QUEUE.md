...
6a2. **Budget variance is WRONG for MONTHLY budgets** (MEDIUM reachable finance-correctness bug, found 2026-07-23). `fin_budget_variance` (0149) aligns actuals to a budget line's period by `extract(quarter)=period_index` — correct for annual/quarterly, but monthly budgets are a shipped feature (`granularity:"monthly"`, periodIndex 0-12) and for those the variance is systematically wrong (months 5-12 match zero actuals; months 1-4 mis-align to the same-numbered quarter). Variance ALERTS (0182) inherit it. **Fix is clear + mechanical** (align by the budget's own `granularity` — exact SQL in the audit doc); NOT built because it's a finance-view migration that changes reported numbers + needs a live-DB verify. Say the word → I write migration 0191 + a mirror test. Details: `docs/audits/2026-07-23-ground-up-audit-session.md`.
