...
| item 12 (sibling — ELO shell rows) | MED | Members can fabricate their OWN `events`/`after_pitch_summaries`/`coaching_sessions` → self-inflate §3.5 ELO | NOT in 0112 (0112 covers the brain + its audit). Still flagged: the fix is route the SYSTEM-kind event/score emissions through DEFINER/service so members create the shell row but not the graded content — touches the event-emission system, needs design + runtime test |
