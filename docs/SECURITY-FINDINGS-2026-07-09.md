...
| care-error-detail-leak | LOW-MED | the customer widget 500 response (`care/conversations/[id]/messages`) returns `detail: "${err.name}: ${err.message}"` to the customer's browser — internal error text (potentially DB constraint/table names) is customer-visible. INTENTIONAL: the widget consumes `.detail` (CareChatWidget:302/347/354) for its error-recovery diagnostics, which help debug delivery. Design tradeoff, not an accidental leak | not fixed — the widget depends on `detail`; removing breaks recovery diagnostics. Options for you: sanitize to an error CATEGORY (not the raw message) in production, or keep the raw detail only when a debug flag is set. Server log (line 302) already has the full detail regardless |
