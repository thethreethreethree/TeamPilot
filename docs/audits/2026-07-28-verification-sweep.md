...
| `care` code → working extension (AMD-006 L2 end-to-end) | sound — `care`/`elostate` code provisions `plan='pro'` (runtime-verified); `computeEntitlement` returns `status:"active"` for `PAID_PLANS={pro,enterprise}` → extension unlocked, no trial/402. The server gate (`extensionAuth`) uses this pure predicate, so the SOT treats the pilot as active. The care module code actually delivers a working extension |
