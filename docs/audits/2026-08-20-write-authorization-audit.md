...
- **Behavioral diagnostic** (stronger than the structural invariants — verifies the predicate actually blocks):
  ...
  admin JWT under RLS and asserts each schedule/care write is blocked or allowed as designed, plus a STRONG
  cross-tenant isolation check (company A writes a real event → company B sees 0). 10 checks. Re-run after any
  policy/RPC change.
