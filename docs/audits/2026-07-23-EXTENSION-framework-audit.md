...
**Deliberately not touched:** the shared `generateConversationDissect` engine prompt (cross-surface; §3.4 no-drift)
— flagged for a deliberate pass, not a unilateral edit. **Source of truth for loading:** `extension/` (my edits);
`store/dist/` + the zip are git-ignored build artifacts regenerated by `build-store-package.mjs`.
