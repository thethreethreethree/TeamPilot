...
**Deployment-correctness VERIFIED (2026-07-23) — the founder-verified fix reaches the shipped artifact.** Layer-2
concern: John confirmed the fixes by loading the extension from SOURCE (`extension/`); what ships to customers is
the `store/dist/` build + the in-app `/care-extension.zip`. Traced the build: `build-store-package.mjs:33-40`
wipes `dist/` fresh (`rmSync`) and re-copies from `extension/` source every run, and its `INCLUDE` list (line 17)
contains BOTH fixed files (`content.js`, `adapters.js`). `scripts/build-extension-download.mjs:16` imports that
script (so the download zip regenerates from the same fresh dist) and runs on the `prebuild` hook — i.e. on every
`next build`/deploy. Current `dist/content.js` already carries the keyboard-containment fix (`beforeinput` present).
