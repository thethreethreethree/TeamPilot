...
| **Supabase linter ERRORs (founder-surfaced 2026-07-30)** | ✅ FIXED — migration 0203 APPLIED (DB at 0203, verify:live 14/14) | 14 finance views were SECURITY DEFINER on prod (cross-tenant leak risk) despite their defs setting security_invoker — prod drifted; 0203 sets `security_invoker=on` explicitly + RLS-seals `_agent_migrations`. Re-run the Supabase linter to confirm the findings clear. |
| **Sales Coach KPI Analytics system (founder directive 2026-07-30, spec docs/SalesCoach-KPI-System.md)** | ✅ COMPLETE (core) — all phases built + verified (1685 tests green). Refinements + founder-gated items remain (see docs/audits/2026-07-30-kpi-system-residuals.md). | Phase 0: "KPI Analytics" nav item (SalesCoachShell) + honest framework page /dashboard/sales-coach/kpi (4 layers, self-baseline philosophy, Reliance Reduction headline, all metrics "building" — no fabricated numbers). Phase 1 foundation (0205 APPLIED, verify:live 14/14): the KPI system DERIVES from existing coaching_sessions/transcript_segments/cues/cue_outcomes/after_pitch_summaries.scores + self-ELO; 0205 adds only what's missing — coaching_sessions.outcome+deal_value (Layer-1 source, founder chose in-app capture) + the computed layer agent_baseline/kpi_snapshot/growth_record (RLS-sealed: read own/manager, NO member write → service-role jobs only, mirrors 0113 anti-self-fabrication). Founder-confirmed decisions: (1) in-app session-outcome capture, (2) score via existing chooseProvider abstraction, (3) reuse after_pitch scores for Layer 3. REMAINING: outcome-capture UI + route; baseline/snapshot computation jobs (idempotent, Understanding-Gate min-sample); reporting surfaces (agent self-baseline + manager rollup + drill-down); Layers 1-4 metric computation. Build per spec order, propose-then-confirm per phase. |
| **COMPREHENSIVE settings system for both modules (founder directive 2026-07-30, full spec APPROVED)** | 🏗️ IN PROGRESS — pillar 1 built. | Founder clarified the real ask: a COMPREHENSIVE settings system for C.A.R.E + Sales Coach (mirroring the substantial main Elostate settings), NOT the two small additions I first shipped. Approved spec — C.A.R.E: General · **AI & Personality** · Appearance & Widget · Channels · Team & Routing · Taxonomy · Learning+Experience · Notifications · Data & Privacy · Account. Sales Coach: General · Methodology & Product · Voice & Live Cues · Grading & Feedback · Learning+Experience · Notifications · Team & Roles · Data & Retention · Account. **Pillar 1 DONE (tsc 0, committed ff4b564c):** dedicated `/settings/ai` "AI & Personality" tab (first tab + first landing card) — moves the buried persona/guidance/knowledge off the bottom of Widget; Widget Save trimmed to disjoint fields (no clobber). **Pillar 2 DONE (tsc 0):** `/settings/general` tab (Learning + Experience mode + a jump-map to all C.A.R.E config); Experience Mode consolidated from the landing. **Sales-Coach General SKIPPED** — its Account tab ALREADY has Learning + Experience mode (verified). **Pillar 3 DONE (tsc 0, careNotify 8/8, 0204 APPLIED, verify:live 14/14):** C.A.R.E Notifications — per-user opt-out `profiles.care_notify_customer_reply` (default true), `/api/me/care-notifications` (A34-guarded), wired into `careNotify.ts` (reads pref, skips push on false, degrades to send if column missing), toggle on the General tab. C.A.R.E had exactly ONE push event (assigned-agent customer-reply), so one real toggle (not a fake multi-event UI). REMAINING (need founder input to avoid dead/wrong surface): C.A.R.E Data & Privacy (retention policy = a data-deletion decision), Sales-Coach Grading & Feedback (scale/cadence). Tracked in the session todo. |
| **"I don't see the C.A.R.E / Sales-Coach settings" (founder, 2026-07-30 ×3)** | 🔎 DIAGNOSED — NOT deploy/cache. AMD-006 L3 DISCOVERABILITY defect (→ fixed by the AI tab above). | Ruled out: stale branch (0 unpushed), failed build (Vercel green, f40e8ff live), missing route (401=exists), C.A.R.E role-gate (panels UNGATED at widget/page.tsx:819/821), config guard (rest of page renders), service worker (never caches shell), HTML cache (`max-age=0, must-revalidate`, Age:0 → reload always fresh + hashed chunks). Founder access CONFIRMED via live DB read (role-check): company_role="admin", sales_coach_role="admin" → isManager=TRUE → Coaching tab IS visible to the founder. So NOT a role gate either. ROOT CAUSE = PLACEMENT + likely STALE-OPEN-SPA: (a) C.A.R.E Jeff-guidance + upload panels are ONLY on the **Widget** tab (5 tabs: Agents/Tags/Shortcuts/Widget/Account), at the **bottom** under "AI personality" — a founder not on Widget / not scrolling past appearance+branding sees the old-looking page. (b) Sales-Coach uploads are on the **Coaching** tab (founder CAN see it), not the default **Account** tab. (c) A SPA kept open since before the 07-30 deploy runs old chunks until a FULL reload (client-side nav doesn't refetch HTML; HTML itself is `max-age=0`). **The features work + the founder has access; the path to them doesn't advertise them + a stale open tab masks them.** FIX (AMD-006 L3, founder-gated per guide-don't-overtake): raise the AI-personality/knowledge panels' discoverability (own tab, or top-of-Widget, or a signpost). |
